...
class PredLLM:
    ...
    def sample_imb(self, n_samples: int, minority_class,
                   max_length: int = 100, task: str = "classification") -> pd.DataFrame:
        ...
        # fix NA values
        # replace NA in labels with the minority class
        X_y_train_new.iloc[:, -1].fillna(minority_class, inplace=True)
        # replace NA in continuous columns with the mean value
        for i_num_cols in self.num_cols:
            X_y_train_new[i_num_cols].replace([np.inf, -np.inf], np.nan, inplace=True)
            imputed_val = round(self.data[i_num_cols].mean(), 2)
            X_y_train_new[i_num_cols].fillna(imputed_val, inplace=True)
        ...
        for i_cat_cols in self.cat_cols:
            ...
            freq_vals = np.array(list(frequencies.values()))
            imputed_val = freq_keys[np.argmax(freq_vals)]
            invalid_indices = np.where(~X_y_train_new[i_cat_cols].isin(freq_keys))[0]
            for idx in invalid_indices:
                X_y_train_new.loc[idx, i_cat_cols] = imputed_val
        ...
        max_length += 10  # fix error of requiring a larger max_length in querying the LLM
        prompts = []
        ...
        target_name = y_train_gen.columns[-1]
        y_train_new = y_train_gen.iloc[:, -1].to_numpy(dtype=float).reshape(-1, )
        ...
        # retain only minority samples
        X_train_new = X_train_new[y_train_new == minority_class]
        y_train_new = y_train_new[y_train_new == minority_class]
        X_train_new = X_train_new.reset_index(drop=True)
        # return minority synthetic samples in a data frame
        X_y_train_new = X_train_new
        X_y_train_new[target_name] = pd.Series(y_train_new)
