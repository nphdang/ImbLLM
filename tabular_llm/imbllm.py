...
from tabular_llm.imbllm_dataset import ImbLLMDataset, GReaTDataCollator
...
class ImbLLM:
    ...
    def fit(self,
            ...
            cont_data: tp.Union[pd.DataFrame, np.ndarray],
            column_names: tp.Optional[tp.List[str]] = None,
            numeric_cols: tp.Optional[tp.List[str]] = None,
            categorical_cols: tp.Optional[tp.List[str]] = None,
        ...
        cont_df = _array_to_dataframe(cont_data, columns=column_names)
        self.data = df
        ...
        df_new = pd.concat([df, cont_df], axis=0)
        df_new = df_new.reset_index(drop=True)
        great_ds = ImbLLMDataset.from_pandas(df_new)
        great_ds.get_ds_size(ds_len=df.shape[0], no_cont=len(numeric_cols))
    ...
    def sample_imb(self, n_samples: int, minority_cond, minority_class,
        ...
        with tqdm(total=n_samples) as pbar:
            ...
            _cnt = 0
            while n_samples > already_generated:
                feature_tokens = great_start.get_start_tokens(k)
                minority_tokens = [minority_cond] * k
                start_tokens = []
                if start_col == "" and start_col_dist is None:
                    start_tokens = minority_tokens
                else:
                    for idx in range(k):
                        start_tokens.append(np.append(minority_tokens[idx], feature_tokens[idx]))
